...
class AndroidReview_Activity : AppCompatActivity() {
    var materials = arrayListOf<AndroidMaterials>()
...
//                "}\n"))
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //RecyclerView
        myRV = findViewById(R.id.rvMain)
        myRV.adapter = RecyclerViewAdapter_Android(materials, this,this)
        myRV.layoutManager = LinearLayoutManager(this)
        readFromDB()
        getSupportActionBar()?.setTitle("Android Review")
    }
            ...
            0 -> {
                ...
                dialogView.cvAdd.setOnClickListener {
                    ...
                    if (reviewTitle.isNotEmpty() && shortDecryption.isNotEmpty() && longDecryption.isNotEmpty()) {
                        ...
                        materialsDatabase.materialsDao().insertAndroidReview(AndroidMaterials(0,reviewTitle,shortDecryption,longDecryption))
    ...
    fun readFromDB() {
        CoroutineScope(IO).launch {
            materials.clear()
            materials.addAll(
                materialsDatabase.materialsDao().getAndroidMaterials()
            )
            withContext(Main) {
